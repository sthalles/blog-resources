...
@tf_export('keras.layers.SpectralNormalizationConv2D')
class SpectralNormalizationConv2D(Conv2D):
    ...
    def __init__(self,
        ...
        super(Conv2D, self).__init__(
            ...
            bias_constraint=constraints.get(bias_constraint),
            **kwargs)
        # initial estimation of singular vector u
        self.u = K.random_normal_variable([1, filters], 0, 1) # [1, out_channels]

    def _l2normalize(self, v, epsilon=1e-12):
        return v / (K.sum(v ** 2) ** 0.5 + epsilon)

    def power_iteration(self, W, u, Ip=1):
        '''
        ...
        for i in range(Ip):
            _v = self._l2normalize(K.dot(_u, W))
            _u = self._l2normalize(K.dot(_v, K.transpose(W)))
    ...
    def compute_spectral_normal(self, training=True):
        ...
        sigma, u, v = self.power_iteration(W_mat, self.u)
    ...
    def call(self, inputs, training=None):
        ...
        #self.kernel = self.compute_spectral_normal(training)
